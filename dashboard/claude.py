...
app.layout = html.Div([
    ...
    dbc.Container([
        html.H1("Data Analysis Tool", className="my-4"),
        
        # Page selection tabs
        dbc.Tabs([
            dbc.Tab(label="Sensitivity Analysis", tab_id="sensitivity-analysis"),
            dbc.Tab(label="Uncertainty Propagation", tab_id="uncertainty-propagation"),
        ], id="tabs", active_tab="sensitivity-analysis", className="mb-4"),
        ...
        return html.Div([
            dbc.Row([
                dbc.Col([
                    dbc.Card([
                        dbc.CardBody([
                            html.H4("Sensitivity Analysis Configuration"),
                            html.Hr(),
                            ...
                            dbc.FormGroup([
                                ...
                                dcc.Dropdown(
                                    ...
                                    options=[{'label': 'None', 'value': 'None'}] + 
                                           [{'label': col, 'value': col} for col in all_columns],
                                    value='None'
                                )
                            ]),
        ...
        return html.Div([
            dbc.Row([
                dbc.Col([
                    dbc.Card([
                        dbc.CardBody([
                            html.H4("Uncertainty Analysis Configuration"),
                            html.Hr(),
                            
                            # Parameter selection
                            dbc.FormGroup([
                                dbc.Label("Select Parameter to Analyze"),
                                dcc.Dropdown(
                                    id='parameter-select',
                                    options=[{'label': col, 'value': col} for col in numeric_columns],
                                    value=numeric_columns[0] if numeric_columns else None
...
@app.callback(
    Output('monte-carlo-options', 'children'),
    [Input('group-column', 'value')]
)
...
@app.callback(
    [Output('sensitivity-plot', 'figure'),
     Output('sensitivity-stats', 'children')],
...
def update_sensitivity_analysis(x_var, y_var, filter_col, filter_val, stored_data):
    ...
    fig = px.scatter(filtered_df, x=x_var, y=y_var, trendline="ols",
    ...
    stats_table = dbc.Table.from_dataframe(
        filtered_df[[x_var, y_var]].describe().reset_index().rename(columns={'index': 'Statistic'}),
        striped=True,
        bordered=True,
        hover=True
    )
...
@app.callback(
    [Output('uncertainty-plot', 'figure'),
     Output('uncertainty-stats', 'children')],
...
def update_uncertainty_analysis(parameter, group_col, group_val, n_samples, stored_data):
    ...
    if group_col != 'None':
        ...
        if group_val is not None:
            ...
            fig = px.histogram(filtered_df, x=parameter, marginal="box", 
                              title=f"Distribution of {parameter} for {group_col}={group_val}")
            
            # Statistics for selected group
            stats_df = filtered_df[parameter].describe().reset_index().rename(columns={'index': 'Statistic', parameter: 'Value'})
            ...
            fig = px.box(df, x=group_col, y=parameter, 
                        title=f"Distribution of {parameter} by {group_col}")
            
            # Statistics by group
            group_stats = df.groupby(group_col)[parameter].agg(['mean', 'std', 'min', 'max']).reset_index()
            
            stats_content = html.Div([
                html.H5("Statistics by Group"),
                dbc.Table.from_dataframe(group_stats, striped=True, bordered=True, hover=True)
            ])
    else:
        # Overall distribution analysis
        fig = px.histogram(df, x=parameter, marginal="box", 
                          title=f"Distribution of {parameter}")
        
        # Calculate basic statistics
        stats_df = df[parameter].describe().reset_index().rename(columns={'index': 'Statistic', parameter: 'Value'})
        ...
        if n_samples:
            ...
            fig2 = px.histogram(x=simulated, marginal="box", 
                               title=f"Monte Carlo Simulation for {parameter} (n={n_samples})")
            
            # Add vertical lines for mean and confidence intervals
            fig2.add_vline(x=mean, line_dash="dash", line_color="red", 
                          annotation_text=f"Mean: {mean:.4f}", annotation_position="top right")
            fig2.add_vline(x=mean + 1.96*std, line_dash="dash", line_color="green", 
                          annotation_text=f"95% CI Upper: {mean + 1.96*std:.4f}", annotation_position="top right")
            fig2.add_vline(x=mean - 1.96*std, line_dash="dash", line_color="green", 
                          annotation_text=f"95% CI Lower: {mean - 1.96*std:.4f}", annotation_position="top left")
            ...
            stats_content = html.Div([
                ...
                html.Div([
                    ...
                    html.P(f"95% Confidence Interval: [{mean - 1.96*std:.4f}, {mean + 1.96*std:.4f}]"),
